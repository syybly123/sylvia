...
const listData = (productList: Product[]) =>
  productList.map((p) => ({
    ...
    tags: (
      <>
        {p.departureCity && <Tag color="#f50">Depart From {p.departureCity}</Tag>}
        {p.travelDays && <Tag color="#108ee9">{p.travelDays} Days </Tag>}
        {p.discountPresent && <Tag color="#87d068">Low Prices</Tag>}
  ...
  return (
    <List
      ...
      footer={
        paging && 
        (<div>
          Total Tourist Routes: <Text strong>{paging.totalCount}</Text> 
        </div>)
      }
      renderItem={(item) => (
        <List.Item
          ...
          <List.Item.Meta
            title={
              <>
                ...
                <Link to={"/detail/" + item.id} style={{cursor:"pointer"}}> {item.title}</Link>
              </>
            }
